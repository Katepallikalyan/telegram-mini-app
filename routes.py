...
# Wrap initialization in a try/except to prevent serverless function crashes
try:
    hathor_client = HathorClient()
except Exception as e:
    logging.warning(f"HathorClient initialization error: {str(e)}")
    # Create a basic placeholder client that won't crash routes on Vercel
    hathor_client = None
...
def index():
    ...
    try:
        ...
        logging.warning(f"Error in index route: {str(e)}")
        # Return a simplified template that will work even if the database fails
        return """
        <html>
        <head>
            <title>Learn & Earn Telegram Mini App</title>
        ...
        <body class="bg-dark text-light">
            <div class="container py-4">
                ...
                <p>Welcome to our blockchain education platform!</p>
                <p>Learn about blockchain technology and earn Hathor tokens as rewards.</p>
                <div class="alert alert-info">
                    Initializing application data... Please check back in a moment.
                </div>
            </div>
        </body>
        </html>
...
def submit_quiz():
    ...
    if score >= quiz.min_score_for_reward:
        ...
        if user and user.hathor_address:
            ...
            try:
                # Check if Hathor client is available
                if hathor_client:
                    ...
                    # In Vercel environment, simulate transaction for demo
                    transaction_id = f"demo-tx-{datetime.utcnow().strftime('%Y%m%d%H%M%S')}"
                    logging.warning("Using simulated transaction ID in serverless environment")
...
def initialize_data():
    try:
        ...
        if Quiz.query.count() == 0:
            ...
            # Create governance topic - handle date carefully to avoid Vercel issues
            now = datetime.utcnow()
            end_date = datetime(now.year, now.month, min(now.day + 7, 28), now.hour, now.minute)
            
            topic = GovernanceTopic(
                title="Next Quiz Topic",
                description="Vote on the next quiz topic to be added",
                status="active",
                end_date=end_date  # Safer date calculation
            )
            ...
            db.session.commit()
            logging.info("Successfully initialized sample quiz data")
    except Exception as e:
        db.session.rollback()
        logging.warning(f"Error initializing data: {str(e)}")
        # Don't raise the exception - let the app continue without sample data
